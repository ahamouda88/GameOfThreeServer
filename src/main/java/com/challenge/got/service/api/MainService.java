...
interface MainService<T, E extends Serializable> {
	...
	default T createOrUpdate(@Valid T model) {
		if (model == null) {
			throw new IllegalArgumentException("Failed to create or update model, model should not be null");
		}
	...
	default T getById(E modelId) {
		if (modelId == null) {
			throw new IllegalArgumentException("Unable to find model, model id should not be null");
		}
